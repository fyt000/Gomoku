...
public class Wuziqi extends JPanel{
		...
		if (totalTurns==225){
			JOptionPane
					.showMessageDialog(null,
							"Game is tie. White wins, since disadvantage of going second.");
			gameOver=true;
			tie=true;
			return true;
		}
	...
	*/
		...
		*/
		...
		for (int i=xMin-1;i<=xMax+1;i++)
			for (int j=yMin-1;j<=yMax+1;j++){
				if (getBoard(i,j)==-1)
					val=val+aiCoe*typeScoreY(findType(i,j,aiColour))-pCoe
							*typeScoreY(findType(i,j,pColour));
			}
	...
	private static int typeScore(int c){ //give score according to the type
	...
	protected int[] pieceCounterX(int x,int y,int direction,int s){ //new counter, more powerful than the one last year
		...
		*/
		if (!isSpaceRight&&!isSpaceLeft){ //no space
			...
			if (lp+rightSpace-1==3&&leftClose>0||rp+leftSpace-1==3
					&&rightClose>0)//find 3 open
				return new int[]{3,2};
			return new int[]{3,1};
			...
			if (lp+rp-1<5) //add together smaller than 5
				...
				if (isSpaceLeft
						&&(rp+leftSpace-1==4&&rightClose==0||leftSpace==4))
					return new int[]{4,2}; //open 4
				if (isSpaceRight
						&&(lp+rightSpace-1==4&&leftClose==0||rightSpace==4))
					return new int[]{4,2};//open 4
