...
public class SettingFrame extends JDialog implements ActionListener{
	...
	public SettingFrame(Frame owner,String title){
		...
		setLocation(Toolkit.getDefaultToolkit().getScreenSize().width/2-200,
				Toolkit.getDefaultToolkit().getScreenSize().height/2-80); //set dialog to the middle of the screen
		//initial settings
		try{
	...
	public void actionPerformed(ActionEvent e){
		...
		if (cmd.equals("ok")){ //ok buttone pressed
			...
			try{ //write everthing into the config.ini file
				...
				System.out
						.println("Unknown error caused by Printing Stream into file config.ini");//may happen if on CD
			}
			setVisible(false);
			//close frame
		}
